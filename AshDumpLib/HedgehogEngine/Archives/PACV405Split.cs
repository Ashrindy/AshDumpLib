...
public class PACV405Split
{
    ...
    public void Write(ExtendedBinaryWriter writer)
    {
        ...
        if (Dependencies.Count > 0)
        {
            ...
            foreach (var i in Dependencies)
            {
                ...
                writer.AddOffset($"chunks{i.GetHashCode()}{i.compressedSize}");
            }
            foreach(var i in Dependencies)
            {
                writer.SetOffset($"chunks{i.GetHashCode()}{i.compressedSize}");
    ...
    public class FileNode : IExtendedBinarySerializable
    {
        ...
        public void Read(ExtendedBinaryReader reader)
        {
            ...
            flags = reader.Read<short>();
            splitId = reader.Read<short>();
            if (dataPtr != 0)
